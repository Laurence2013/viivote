    ...
    def get_questions(self):
        ...
        for ask_id in range(0,len(get_ask_id)):
            for get_id in get_ids:
                if get_id.get('question_id_id') == get_ask_id[ask_id].get('id'):
                    get_qs_ids = Ask_A_Question_table.objects.filter(id = get_id.get('question_id_id')).values('id','question','date_updated').latest('date_updated')
